...
class Ui_MainWindow(object):
    ...
    def open_file(self, button_name):
        ...
        # Read parameters from csv file
        self.duration, self.freq_list, self.power_list, self.error, self.msg  = rcsv.read_and_plot(opened_path, self.enable_plot.isChecked(), False)
        # print("The error values is: {}".format(error))
        # print("The return msg is {}".format(msg))
        self.outputLabel.setText(self.msg)
        if not self.error:
    ...
    def play_execution(self):
        ...
        while time.time()-init_time <= sum(self.duration) and not exit_flag:
            ...
            self.update_values_on_screen(MainWindow, time.time())
            self.save_error_log()
            # update_values()
        # Soft turn off
    ...
    def update_values_on_screen(self, MainWindow, timestamp):
        if timestamp - self.timestamp_old >= 1: # update at least 1 second after the last one
            _translate = QtCore.QCoreApplication.translate
            print("###### Update values on screen!")
            self.temperature_label.setText(_translate("MainWindow", str(self.status["Temperature"])))
            self.pll_label.setText(_translate("MainWindow", str(self.status["PLL"])))
            self.current_label.setText(_translate("MainWindow", str(self.status["Current"])))
            self.voltage_label.setText(_translate("MainWindow", str(self.status["Voltage"])))
            self.reflectedpower_label.setText(_translate("MainWindow", str(self.status["Reflected Power"])))
            self.forwardpower_label.setText(_translate("MainWindow", str(self.status["Forward Power"])))
            self.pwm_label.setText(_translate("MainWindow", str(self.status["PWM"])))
            self.onoff_label.setText(_translate("MainWindow", str(self.status["On Off"])))
            self.enablefoldback_label.setText(_translate("MainWindow", str(self.status["Enable foldback"])))
            self.foldbackin_label.setText(_translate("MainWindow", str(self.status["Foldback in"])))
            self.erro_label.setText(_translate("MainWindow", str(self.status["Error"])))
